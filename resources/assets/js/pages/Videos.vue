    ...
    <div class="l-videos">
        ...
        <div class="l-videos__toolbar">
            <div class="action" @click="loadMore" :class="{ disabled: !hasMoreLoad }">
                <i class="fas fa-sync-alt"></i><span>加載更多</span>
            </div>
        </div>

        <div class="l-videos__recommandations" ref="recommandations">
            <div class="card l-videos__selection-item" v-for="(video, i) in videos" :key="video.video_url + i" @click="switchVideo(video)">
...
export default {
  ...
  methods: {
      ...
      loadMore() {
          axios.get(`/videos?page=${this.paginator.current_page + 1}`)
            .then(({ data }) => {
                this.paginator = data;
